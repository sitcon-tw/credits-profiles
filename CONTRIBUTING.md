...
我是否已經用 SITCON Credits 網頁標記了可能是在記錄自己的公開貢獻紀錄，想請維護者協助確認：
<請貼上標記後的頁面網址，或填「不用」；如果不確定，請先問我>

如果我提供了標記網址，請用下面方式協助我整理 Pull Request 裡的「貢獻紀錄標記網址（選填）」段落：

1. 優先使用我貼上的標記網址，不要改寫成身份確認結論。
2. 只能把候選紀錄列成「請維護者確認的線索」。不要判定那一定是我，不要宣稱身份已確認，也不要宣稱這會自動修改 Google Sheets。
3. 如果我沒有提供標記網址，請引導我打開 [標記我的貢獻紀錄](http://sitcon.org/credits/?claim=1) 搜尋與標記；不要自己搜尋姓名、暱稱、GitHub username 或其他個人資料。
4. Pull Request 說明中只填 PR template 要求的標記網址；不知道或讀不到時留空並寫「請維護者協助確認」。
