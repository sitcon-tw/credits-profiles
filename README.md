...
- `CI`：在 `master` push 與手動觸發時執行 `pnpm test` 與 `pnpm profiles:validate`。一般 `pull_request` CI 不啟用，避免 fork PR 停在 workflow approval；profile PR 由下列可信任檢查驗證，支援檔案與 workflow 變更則需要維護者 review。
