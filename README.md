...
```text
請協助我在 SITCON Credits Profiles 儲存庫新增或更新我自己的公開 profile，並協助我送出 Pull Request；如果你目前的環境不能直接送 Pull Request，請改成提供我可以用 GitHub 網頁手動送出的內容與步驟。
...
4. 如果你能在本機修改檔案，完成後請執行 `pnpm profiles:validate`。如果儲存庫文件要求，請再執行 `pnpm test`。
